...
from llm_features import LLMFeatures
...
# ---------- LLM profile embeddings ----------

print("Loading LLM profile embeddings...")
llm = LLMFeatures(
    project_root=PROJECT_ROOT,
    user_id_to_dense=user_id_map,
    book_id_to_dense=book_id_map,
    n_users=n_users,
    n_books=n_books,
)
print(f"  user emb coverage: {llm.user_coverage*100:.1f}%  ({int(llm.user_coverage*n_users):,}/{n_users:,})")
print(f"  item emb coverage: {llm.item_coverage*100:.1f}%  ({int(llm.item_coverage*n_books):,}/{n_books:,})")
...
print("Precomputing LLM history embeddings (mean of read-item embs per user)...")
llm.precompute_history_embs(train_user_items)
...
ranker_pool = llm.attach(ranker_pool)
...
for start in range(0, len(test_user_ids), CHUNK):
    ...
    chunk_pool = llm.attach(chunk_pool)
